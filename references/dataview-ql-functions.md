...
Complete reference for all functions available in Dataview Query Language (DQL).

## Overview

Dataview functions provide advanced ways to manipulate data in queries. Functions can be used in:
- Data commands (WHERE, SORT, GROUP BY, FLATTEN, LIMIT)
- TABLE columns for calculations
- LIST additional output
...
- [Constructors](#constructors) - `object()`, `list()`, `date()`, `dur()`, `number()`, `string()`, `link()`, `typeof()`
- [Numeric Operations](#numeric-operations) - `round()`, `min()`, `max()`, `sum()`, `average()`, etc.
- [Objects, Arrays, and String Operations](#objects-arrays-and-string-operations) - `contains()`, `sort()`, `filter()`, `map()`, etc.
- [String Operations](#string-operations) - `replace()`, `split()`, `substring()`, `regextest()`, etc.
- [Utility Functions](#utility-functions) - `default()`, `choice()`, `dateformat()`, `meta()`, etc.

---

## How Functions Work

Functions are expressions that always return a new value. They follow this format:

functionname(parameter1, parameter2)
Parameters are again expressions and you can use literals, meta data fields, or even another function as parameter. You'll find out which data type your parameters need to have on the documentation of this page. Pay attention to the information inside the function brackets. Parameters in square brackets, i.e. link(path, [display]) means they are optional and can be omitted. Find out more about the default behavior of each function on their explanation.

### Function Vectorization

Most functions can be applied to either single values OR lists of values. When applied to a list, the function is applied to each element and returns a new list:
...
This "function vectorization" applies implicitly to most functions and won't be mentioned for each function below.

---

## Constructors

Create new values of specific types.
...
- [Luxon Tokens](https://moment.github.io/luxon/#/formatting?id=table-of-tokens) - For dateformat() and durationformat()
